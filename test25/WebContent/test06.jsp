...
<%
	...
	out.println("<h3>page(페이지)는 그 정보가 현재 페이지 내에서만 유지된다.</h3>");
	
	out.println("<h3>전송된 값은 그 정보가 목적지까지만 유지되며, 다른 페이지에서는 해당 정보를 확인할 수 없음</h3>");
	out.println("<p>웹 환경에서의 Scope : 그 정보가 어디 또는 언제까지 유지</p>");
	out.println("page < trans(request & response) < session < application");
	
	//page.setAttribute("name","정도준");		-> 모든 8대 객체에 값을 저장하는 경우는 setAttribute("필드명", "값")
	//response.setAttribute("name","정도준");	-> 모든 8대 객체에서 값을 가져올 경우는 getAttribute("필드명") 단, request 객체의 값을 가져올 경우는 getParameter("파라미터명")을 사용한다.
%>
<hr>
<div>
