...
class ManagerLogin extends JFrame implements ActionListener, KeyListener {	//GUI 관리자 창
	...
	@Override
	public void keyTyped(KeyEvent e) {	//특정 키를 눌렀다가 떼었을 때
	...
	public void keyPressed(KeyEvent e) {	//키보드 입력 시 검증
		try {
			if(e.getKeyCode() == KeyEvent.VK_ENTER) {	//e : event
	...
	@Override
	public void keyReleased(KeyEvent e) {	//키보드의 특정 키에서 떼었을 때
...
class ChatServerGUI extends JFrame implements ActionListener, KeyListener {
	...
	TextArea serverChatList = new TextArea(30, 50);	//TextArea(줄, 칸)
	TextArea userList = new TextArea(30, 15);
	
	TextField portLabel = new TextField(20);
	TextField ipLabel = new TextField(40);
	...
	public ChatServerGUI(int port) {
		...
		try {
			ipLabel.setText("현재 IP : "+InetAddress.getLocalHost());
		} catch (UnknownHostException e) {
			e.printStackTrace();
		}
		portLabel.setText("현재 연결 포트 : "+port);
